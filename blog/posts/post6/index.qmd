...
Likewise, we obtain a Gaussian profile when the mixing parameters $\eta_L = \eta_I = \eta_P = 0$, while the Lorentzian component is controlled by paramater $\eta_L$. We obtain a pure Lorentzian profile for $\eta_L = 1$. These parameters are constrained to vary between 0 and 1 and must satisfy the condition $\eta_L + \eta_I + \eta_P = 1$, the parameters represent a simplex in three dimensions.
...
![](epv_animation.gif){fig-align="center"}
