...
#| code-fold: true
#| code-summary: "Show the code"
wold_osc_filter <- wold_osc(scale(x_matrix, scale = FALSE), y_target, ncomp = 10, tol = 1e5, max.iter = 10)
wold_osc_scores <- wold_osc_filter %>% pluck("scores") %>% as_tibble() %>% mutate(extract = y_target)
```
...
wold_osc_ellipse <- ellipseParam(wold_osc_scores, k = 2, pcx = 1, pcy = 2)
p2 <- wold_osc_scores %>%
  ggplot() +
  ggforce::geom_ellipse(
    aes(
      x0 = 0, 
      y0 = 0, 
      a = pluck(wold_osc_ellipse, "Ellipse", "a.95pct"), 
      b = pluck(wold_osc_ellipse, "Ellipse", "b.95pct"),
      angle = 0), 
    linewidth = .5, linetype = "solid", fill = "white") +
  aes(x = V1, y = V2, fill = extract) +
  geom_point(size = 5L, shape = 21, colour = "#112446") +
...
wold_osc_loadings <- wold_osc_filter %>% 
  pluck("loadings") %>%
  as_tibble() %>%
  mutate(wavelength = rep(seq(1100, 2250, 2), times = 1))
```
...
```{r echo=FALSE}
p3 <- beer %>% mutate(spectra_id = paste0("s", 1:80)) %>%
  pivot_longer(
...
p4 <- ggplot() +
  geom_line(data = wold_osc_loadings, aes(x = wavelength, y = V1, color = "Wold's OSC"), linewidth = 0.8) +
  geom_line(data = osc_loadings, aes(x = wavelength, y = V1, color = "Sjöblom's OSC")) +
...
(
  ...
  (p4 + theme(legend.position = "right", plot.margin = margin(t = 0)))
```
